...
**Le package** [`Rzine`](https://gitlab.huma-num.fr/rzine/package) **fournit le modèle de mise en page des fiches Rzine**, ainsi que l'ensemble des fichiers associés à fournir.
...
Vous pouvez générer une fiche `readrzine` vierge en ligne de commande. Pour cela, **copiez-collez les lignes de code suivantes sans les modifier, puis exécutez :**
...
Vous pouvez **kniter (générer) le R Markdown `readrzine` en format HMTL** de différente façon :

**1- Ligne de commande**

Copiez-collez ces lignes de commande, personnalisez le chemin puis exécutez pour générer le fichier HTML :

```r
rmarkdown::render(".../Ma_fiche_Rzine/Ma_fiche_Rzine.Rmd", envir = new.env())
```
**2- Clic- bouton avec Rstudio**
...
**Cela engendre la création d'un fichier du même nom mais en format HTML** (***index.html***) **à la racine de votre répertoire. Il s'agit du fichier Rmd mis en page à partir du modèle** `readrzine` :
...
Le template `readrzine` fonctionne comme n'importe quel R Markdown. 



Cependant certaines métadonnées et certains contenus sont à renseigner obligatoirement et une certaine mise en page est à respecter.
