...
Une fois le processus d'évaluation terminée, les éditeur·rice·s s'occupent de la mise en page finale de la fiche, du dépôt d'un [DOI](https://www.doi.org/), du référencement du document sur HAL et de sa mise en ligne sur le site [rzine.fr](https://rzine.fr/publication_rzine/).
...
Bien que dans le cadre d'une publication Rzine certaines métadonnées et informations sont à renseigner et que certaines règles de mise en page doivent également être appliquées, le modèle de mise en page **`readrzine`** fonctionne comme n'importe quel autre fichier R Markdown.

<div class="alert alert-success" role="alert">
<b>Ce chapitre est introduction au langage de balisage Markdown et à la construction de document R Markdown. Il ne vous sera pas utile si vous en maîtrisez déjà les différents aspects.</b></div>
...
**Les données utilisées sont centrales dans une fiche Rzine. Plusieurs consignes sont à respecter.**
...
<div class="alert alert-danger" role="alert">
<b>Les données mobilisés doivent pourvoir être librement diffusées et utilisées par tous les lecteur·rice·s. Il est donc très important de vérifier leur conditions d'utilisations.</b></div>
...
<div class="alert alert-danger" role="alert">
<b>L'utilisation d'un package doit être justifiable. Les éditeur·rice·s seront particulièrement attentifs à l'utilisation de packages récent et mis à disposition via un logiciel de gestion de versions.</b></div>
...
Cela affichera l'image centrée et large de 200 pixels, ainsi qu'un titre :
...
Il est possible d'utiliser les [messages d'alerte boostrap](https://getbootstrap.com/docs/4.0/components/alerts/). Vous pouvez utilisez deux mise en page différente :   
...
<div class="alert alert-danger" role="alert">Message très important</diV>
...
Trois référence bibliographique sont saisie par défaut dans le fichier *biblio.bib* :
...
**La seule chose à faire est de ne pas modifier ni déplacer pas cette section**. 
...
**Vous pouvez alors personnaliser le fichier** en modifiant : le **titre**, le **sous-titre**, le.s **auteur.e.s** (et **affiliation.s**), ainsi que l'**URL de consultation de la fiche**. Exemple :
...
<div class="alert alert-danger" role="alert">
<b>Rédaction de la documentation à venir.</b></div>
