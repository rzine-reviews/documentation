...
La "fiche" est un des deux formats proposés de la collection Rzine. **Il s'agit d'un document didactique qui présente une méthodologie, un type d'analyse, une fonctionnalité, ou encore le traitement d’un certain type de données avec le logiciel R. Une fiche peut être principalement technique, méthodologique ou thématique, mais elle peut aussi couvrir ces trois aspects**.
...
- [Analyse Territoriale Multiscalaire](https://rysebaert.github.io/mta_rzine/), *Ronan Ysebaert et Claude Grasland*.    
- [Analyse des corrélations avec easystats](https://rzine.fr/docs/20200526_glecampion_initiation_aux_correlations/rapp_correlation.html), *Grégoire Le Campion*.    
...
- [Réaliser une carte de discontinuités en 2,5D](https://neocarto.github.io/ironcurtain/ironcurtain.html), *Nicolas Lambert*.    

**Les auteur·e·s sont libres dans le choix de la problématique et dans la façon d'articuler le contenu**. Les seules obligations en matière de contenu sont :

- **Une fiche doit être rédigée en français ou en anglais**.
- **Les traitements présentés doivent être reproductibles** et respecter le RGPD.
- **Les données utilisées doivent être accessibles**.
- **Une fiche doit contenir une introduction qui contextualise le contenu présenté**.
- **Certaines annexes sont obligatoires** (information sur la session et citation de la fiche) ou fortement recommandés (glossaire et références bibliographiques). 
...
- **Utiliser le template** (et l'architecture de dossier) `readrzine` ([partie 2.1](#installation)).
- **Rédiger en français ou en anglais**
- **Fournir certains fichiers et annexes associés** ([partie 4](#rédiger-une-fiche-rzine)). 
- **Introduire et contextualiser la fiche** ([partie 4.1](#introduction-contextualisation)). 
- **Présenter des traitements reproductibles** et respecter le RGPD.
- **Mettre à disposition les données utilisées** ([partie 4.2](#données-utilisées)). 


**Une fois votre fiche soumise au comité de lecture, elle sera évaluée sur les** [**critères suivants**](https://rzine.gitpages.huma-num.fr/help_rzine_publication/#crit%C3%A8res-d%C3%A9valuation).


## Critères d'acceptation


Afin d'assurer la cohérence de la collection et une certaine qualité du contenu des fiches Rzine, **plusieurs critères d'évaluation ont été établis** . **Certains de ces critères sont soumis à un ou plusieurs relecteur.rice.s, d'autres peuvent faire l'objet d'un refus immédiat de la part du** [**comité de lecture**](https://rzine.fr/collection/#people).  


**Le premier objectif d'une fiche Rzine consiste à partager des méthodes reproductibles, applicables en SHS**. Pour cela, le comité de lecture et les relecteur·rice·s veilleront à ce que la démonstration soit contextualisée (champ disciplinaire d'application, enjeux en terme de reproductibilité...) et rigoureuse. Le code présenté se doit d'être lisible, commenté et reproductible. Les packages mobilisés doivent être brièvement décrits et leur utilisation justifiée. Enfin, les données utilisées dans le cadre de la démonstration doivent obligatoirement être sourcées, libres de droit et mises à disposition.


|  Critères d'évaluations d'une fiche                                |  Comité de lecture | Relecteur.rice.s  |
|--------------------------------------------------------------------|:-------------:|:-------------:|
| Respect des consignes éditoriales ([partie 3](#processus-de-publication))    |       X       |               |
...
| Données utilisées libres, sourcées et mises à disposition          |       X       |       X       |
...
| Clarté (indentation, commentaire...) et reproductibilité du code présenté  |               |       X       |
| Justification (et stabilité) des packages utilisés                 |       X      |       X       |

*Le cas échéant, le comité éditorial se réserve la possibilité de mettre en relation différents contributeur.rice.s pour leur proposer la rédaction collaborative d'une fiche. 
...
Une fiche Rzine est **un fichier HTML produit en R Markdown**. Elle se construit donc avec R et repose sur les packages [rmarkdown](https://rmarkdown.rstudio.com/) et [knit](https://yihui.org/knitr/). 

Une fiche doit respecter une mise en page ainsi qu'une structure prédéfinie de répertoire. Pour faciliter cela, **un modèle pré-rempli est mis à disposition par le package** [**rzine**](https://gitlab.huma-num.fr/rzine/package). L'installation du package et la génération d'une fiche Rzine vierge est expliquée dans la [partie 3.1](#édition) 


Une fiche Rzine est **un fichier HTML produit en R Markdown à partir d'un modèle de mise en page fournit par le package** [**rzine**](https://gitlab.huma-num.fr/rzine/package). Ce modèle de mise en page fournit également toute l'infrastructure de répertoires et les fichiers associés (vierges) demandés à la soumission d'une fiche.

Le package est mis à disposition sur un [dépôt de l'organisation Rzine](https://gitlab.huma-num.fr/rzine/package) sur le GitLab Huma-Num.   
Pour l'installer, exécutez les lignes suivantes :
...
Vous pouvez ensuite générer une fiche Rzine vierge en clic-bouton depuis l'interface Rstudio (File > New File > R Markdown > From temmplate) ou utiliser la fonction suivante :
...
**Retrouvez l'ensemble du processus d'édition expliqué et détaillé dans** [**ce document**](https://rzine.gitpages.huma-num.fr/Help_Rzine_Layout/), @RedactRzine.
...
**Rzine souhaite promouvoir l'usage de logiciels contribuant à une science plus ouverte et reproductible**. L'analyse, le code et les données doivent être publics et accessibles depuis un logiciel de gestion de version décentralisé. Ainsi, pour qu'une fiche soit acceptée en relecture, **elle doit impérativement être mise à disposition sur un Git (GitHub ou GitLab) et déployée, c'est à dire consultable, depuis celui-ci**.

Cela ne doit en aucun cas effrayer les auteur.e.s qui n'ont jamais utilisé ce genre de logiciel. De la documentation et une assistance sont fournies par le comité éditorial Rzine... **Profitez-en pour vous former à cet outil qui change la vie !**

Retrouvez plus de précisions à ce sujet dans la [partie 3.2](#soumission) 

## Soumission

Une fois votre fiche rédigée et générée en format HTML, soumettez-la au comité de lecture.
Pour cela, **vous devez déposer l'ensemble du répertoire sur un dépôt GitHub, puis déployer votre fiche afin de la rendre consultable en ligne**... Vous ne comprenez rien ? Ce n'est pas grave, de la documentation ainsi que l'équipe Rzine sont là pour vous accompagner dans ce processus.

La soumission d'une fiche peut se résumer en quatre étapes :

1) Créer un compte GitHub (si ce n'est pas déjà fait).
2) Créer un nouveau dépôt puis y déposer l'ensemble de votre répertoire.
3) Déployer votre fiche en format HTML pour la rendre consultable en ligne.
4) Envoyer un mail à [contact@rzine.fr](mailto:contact@rzine.fr) pour alerter le comité de lecture de votre soumission.

**Retrouvez toutes les informations nécessaires pour vous accompagner dans le processus de soumission dans** [**cette documentation**](https://rzine.gitpages.huma-num.fr/help_rzine_submit/), @SubmitRzine. 

Au regard de la ligne éditoriale de la collection, **le comité de lecture statue sur la validité d'une soumission sous 15 jours**.


## Évaluation

Chaque fiche publiée dans la collection Rzine fait l'objet d'une relecture par un·e ou de plusieurs expert·e·s de la thématique (ou méthode/technique) et d'un contrôle éditorial par des [membres du comité de lecture](https://rzine.fr/collection/#people).

**Le processus de relecture se veut complètement transparent. L'ensemble des retours, ajouts ou modifications demandés, ainsi que l'identité des relecteur·rice·s sont publics**.

**Ce processus de relecture se fait en intégralité via GitHub ou GitLab**. De la documentation et une assistance sont fournies par le comité éditorial Rzine pour vous aider si nécessaire. Retrouvez plus de précisions à ce sujet dans la [partie 3.3](#relecture)
...
**Si vous n'avez aucune pratique ni connaissance de GitHub, une documentation complète est mise à votre disposition** [**ici**](https://rzine.gitpages.huma-num.fr/help_rzine_reviewing/), @ReviewRzine, **pour vous accompagner dans le processus de relecture**. Les [éditeurs du comité de lecture](https://rzine.fr/collection/#people) pourront également vous assister dans ce processus.

## Publication

Une fois les demandes d’améliorations prises en compte et la publication validée par l'évaluateur·rice, une fiche de métadonnées associée à la publication vous sera demandée afin d'assurer son référencement sur [Rzine.fr](https://rzine.fr/publication_rzine/).

**Votre dépôt est ensuite cloné dans le dépôt GitLab Rzine, et la publication déployée depuis celui-ci. Un DOI sera associé à la publication, qui sera également référencée sur** [**HAL**](https://hal.archives-ouvertes.fr/) **et sur** [**Rzine.fr**](https://rzine.fr/publication_rzine/).

## Processus de publication

**Rzine souhaite promouvoir l'usage de logiciels contribuant à une science plus ouverte et reproductible**. C'est pour cette raison que **le processus de publication mis en place repose sur l'utilisation du logiciel de gestion de versions décentralisé Git**. L'ensemble du code source, la fiche générée et les différentes ressources associées (données, images, citation, références bibliographiques) doivent être intégralement mis à disposition sur un dépôt Git public. La fiche produite doit y être déployée (consultable) et l'ensemble du processus de relecture est effectué à l'aide de fonctionnalités proposées par Git.

**De la documentation est à votre disposition pour vous accompagner dans cette démarche**. Une assistance du comité de lecture est également possible. **Si vous n'êtes pas encore un·e utilisateur·rice de Git, ne fuyez pas ! Profitez-en plutôt pour vous former...**

## Licence d'utilisation

**Attention, être auteur.e d'une fiche Rzine, c'est accepter à ce que votre production soit mise à disposition sous les conditions d'utilisation de la** [**licence Creative Commons BY-SA 4.0**](https://creativecommons.org/licenses/by-sa/4.0/deed.fr).
...
Ainsi, **le lien vers le code source de la fiche** ([dêpot GitLab Humanum Rzine](https://gitlab.huma-num.fr/rzine)) **sera directement indiqué** sur le document par le comité éditorial. 
...
Le package [`Rzine`](https://gitlab.huma-num.fr/rzine/package) met à disposition un template de fiche ainsi que l'ensemble des fichiers associés à fournir.
...
Cela génère un ensemble de fichiers (cf [partie 2.5](#les-fichiers-associés)) à la racine de votre répertoire de travail :
...
## Le R Markdown `readrzine`

Un fichier index.Rmd a été créé à la racine de ce nouveau répertoire :

```{r, echo=FALSE, fig.align='center', out.width = "60%", fig.margin=TRUE}
knitr::include_graphics("figures/readrzine_vierge.png")
```
...
**Ce fichier HTML est automatiquement enregistré à la racine du répertoire source**. De plus, **Un autre fichier nommé** ***cite.bib*** ([cf. partie 4.6](#citation)) **est automatiquement généré à chaque knit du fichier** ***index.Rmd*** :

```{r, echo=FALSE, fig.align='center', out.width = "25%"}
knitr::include_graphics("figures/html_file.png")
```
